pipeline {
    ...
    stages {
        ...
        stage('Get version from latest tag') {
            ...
            steps {
                script {
                    withCredentials([sshUserPrivateKey(credentialsId: 'github_ssh_key', keyFileVariable: 'GIT_SSH_KEY')]) {
                        sh 'GIT_SSH_COMMAND="ssh -i $GIT_SSH_KEY -o StrictHostKeyChecking=no" git fetch --prune --tags origin'
                    }
                    ...
                    if (!env.GITOPS_REPO?.trim()) {
                        ...
                        withCredentials([sshUserPrivateKey(credentialsId: 'github_ssh_key', keyFileVariable: 'GIT_SSH_KEY')]) {
                            sh '''
                                ...
                                rm -rf gitops-tmp
                                git clone $MEDICONY_GITOPS_REPO gitops-tmp
                            '''
                            dir('gitops-tmp/k8s/overlays/medicony') {
                                ...
                                sh 'GIT_SSH_COMMAND="ssh -i $GIT_SSH_KEY -o StrictHostKeyChecking=no" git push'
                            }
                            sh 'rm -rf gitops-tmp'
                        }
                    } else {
